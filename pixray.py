...
try:
    ...
    # update class_table if these import OK
    class_table.update({"line_sketch": LineDrawer, "pixel": PixelDrawer, "clipdraw": ClipDrawer})
except ImportError as e:
...
class MakeCutouts(nn.Module):
    def __init__(self, cut_size, cutn, cut_pow=1.0):
        ...
        if global_aspect_width == 1:
            ...
            augmentations.append(
                MyRandomAffine(
                    degrees=0, translate=(n_t, n_t), scale=(n_s, n_s), p=1.0, return_transform=True
                )
            )
        elif global_aspect_width > 1:
        ...
        augmentations.append(
            K.CenterCrop(size=self.cut_size, cropping_mode="resample", p=1.0, return_transform=True)
        )
        augmentations.append(
            MyRandomPerspectivePadded(distortion_scale=0.20, p=0.7, return_transform=True)
        )
        augmentations.append(K.ColorJitter(hue=0.1, saturation=0.1, p=0.8, return_transform=True))
...
def setup_parser(vq_parser):
    ...
    vq_parser.add_argument(
        "-sp", "--spot", type=str, help="Spot Text prompts", default=[], dest="spot_prompts"
    )
    vq_parser.add_argument(
        "-spo",
        "--spot_off",
    ...
    vq_parser.add_argument(
        "-spf", "--spot_file", type=str, help="Custom spot file", default=None, dest="spot_file"
    )
    vq_parser.add_argument(
    ...
    vq_parser.add_argument(
        "-ip", "--image_prompts", type=str, help="Image prompts", default=[], dest="image_prompts"
    )
    vq_parser.add_argument(
        "-ipw",
        "--image_prompt_weight",
    ...
    vq_parser.add_argument(
        "-il", "--image_labels", type=str, help="Image prompts", default=None, dest="image_labels"
    )
    vq_parser.add_argument(
        "-ilw",
        "--image_label_weight",
    ...
    vq_parser.add_argument(
        "-i", "--iterations", type=int, help="Number of iterations", default=None, dest="iterations"
    )
    vq_parser.add_argument(
        "-se", "--save_every", type=int, help="Save image iterations", default=10, dest="save_every"
    )
    vq_parser.add_argument(
        "-de",
        "--display_every",
    ...
    vq_parser.add_argument(
        "--quality", type=str, help="draft, normal, better, best", default="normal", dest="quality"
    )
    vq_parser.add_argument(
        "-asp", "--aspect", type=str, help="widescreen, square", default="widescreen", dest="aspect"
    )
    vq_parser.add_argument(
        "-ezs", "--ezsize", type=str, help="small, medium, large", default=None, dest="ezsize"
    )
    vq_parser.add_argument(
        "-sca", "--scale", type=float, help="scale (instead of ezsize)", default=None, dest="scale"
    )
    vq_parser.add_argument(
        "-ova",
        "--overlay_alpha",
    ...
    vq_parser.add_argument(
        "-ii", "--init_image", type=str, help="Initial image", default=None, dest="init_image"
    )
    vq_parser.add_argument(
        "-iia",
        "--init_image_alpha",
    ...
    vq_parser.add_argument(
        "-ti", "--target_images", type=str, help="Target images", default=None, dest="target_images"
    )
    vq_parser.add_argument(
        "-anim",
        "--animation_dir",
    ...
    vq_parser.add_argument(
        "-as", "--auto_stop", type=str2bool, help="Auto stopping", default=False, dest="auto_stop"
    )
    vq_parser.add_argument(
        "-cuts", "--num_cuts", type=int, help="Number of cuts", default=None, dest="num_cuts"
    )
    vq_parser.add_argument(
        "-bats",
        "--batches",
    ...
    vq_parser.add_argument(
        "--output", type=str, help="Output filename", default="output.png", dest="output"
    )
    vq_parser.add_argument(
        "--outdir",
        type=str,
...
def process_args(vq_parser, namespace=None):
    ...
    if namespace == None:
        ...
        args = vq_parser.parse_known_args()
    elif isnotebook() or hasattr(namespace, "skip_args"):
        args = vq_parser.parse_known_args(args=[], namespace=namespace)
    else:
        # sometimes there are both settings and cmd line
        args = vq_parser.parse_known_args(namespace=namespace)
    args=args[0]
    ...
    quality_to_scale_table = {"draft": 1, "normal": 2, "better": 3, "best": 4, "supreme": 5}
    # this should be replaced with logic that does somethings
    # smart based on available memory (eg: size, num_models, etc)
    quality_to_num_cuts_table = {"draft": 24, "normal": 30, "better": 36, "best": 12, "supreme": 8}

    quality_to_batches_table = {"draft": 1, "normal": 1, "better": 1, "best": 2, "supreme": 4}
    ...
    if args.learning_rate_drops is None:
        ...
        args.learning_rate_drops = [
            int(map_number(n, 0, 100, 0, args.iterations - 1)) for n in args.learning_rate_drops
        ]
...
def parse_known_args_with_optional_yaml(parser, namespace=None):
    parser.add_argument("--config_file", dest="config_file", type=argparse.FileType(mode="r"))
...
def apply_settings():
    ...
    vq_parser = argparse.ArgumentParser(description="Image generation using VQGAN+CLIP")
    vq_parser.add_argument(
        "--drawer", type=str, help="clipdraw, pixel, etc", default="vqgan", dest="drawer"
    )
    vq_parser.add_argument(
